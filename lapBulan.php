...
<body style="margin-left: 100px;margin-right: 100px;" onload="window.print()">
  <div class="container">
    ...
    <center>
      <h1> Laporan Per <?php echo tgl_indo($_GET['bulan']) ?></h1>
    </center>
      ...
      <div class="table-responsive">
        <table class="table">
          ...
          <tbody>
            ...
            $ambil = $koneksi->query("SELECT * FROM `tb_pembelian` JOIN `tb_pembelian_produk` ON  `tb_pembelian`.`pembelian_id`=`tb_pembelian_produk`.`pembelian_id` JOIN `tb_produk` ON `tb_pembelian_produk`.`produk_id` = `tb_produk`.`produk_id` WHERE  `tb_pembelian`.pembelian_tgl LIKE '%$_GET[bulan]%' ");
